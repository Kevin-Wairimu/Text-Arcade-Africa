 ...
 * src/pages/Admin.jsx
 * -------------------------------------------------
 *  • Suspend/Unsuspend button is now a full‑width,
 *    coloured, active button.
 *  • UI updates instantly after the API call.
 *  • MENU TAB PERSISTS ON REFRESH
 *  • GET /articles NOW SENDS JWT → LOGS USER AS ADMIN
 * -------------------------------------------------
 */
...
export default function Admin() {
  ...
  const [menuOpen, setMenuOpen] = useState(() => {
    ...
    return saved || "dashboard"; // persist tab on refresh
  });
  ...
  useEffect(() => {
    try {
      ...
      if (token)
        console.log("Using token (masked):", `${token.slice(0, 10)}...`);
    } catch (e) {
      /* ignore */
    }
  }, [token]);
  ...
  /* -----------------------
     Fetch: Articles (NOW SENDS TOKEN → LOGS ADMIN)
     ----------------------- */
  async function fetchArticles() {
    ...
    try {
      console.log(
        `Fetching articles from ${API?.defaults?.baseURL || ""}/articles`
      );
      const { data } = await API.get("/articles", {
        headers: { Authorization: `Bearer ${token}` }, // ← NOW SENDS TOKEN
      });
      ...
      setError((prev) => ({
        ...
        articles:
          err.response?.status === 404
            ? `Articles endpoint not found at ${
  ...
  async function fetchUsers() {
    ...
    try {
      ...
      if (usersArray.length === 0) {
        setError((prev) => ({
          ...prev,
          users: "No users found in the database.",
        }));
      }
    } catch (err) {
  ...
  async function fetchSettings() {
    ...
    try {
      console.log(
        `Fetching settings from ${API?.defaults?.baseURL || ""}/settings`
      );
      const { data } = await API.get("/settings", {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      setError((prev) => ({
        ...
        settings:
          err.response?.status === 404
            ? `Settings endpoint not found at ${
                API?.defaults?.baseURL || ""
              }/settings.`
            : err.response?.status === 401
            ? "Unauthorized. Please log in again."
  ...
  async function handleSettingsSubmit(e) {
    ...
    try {
      console.log(
        `Saving settings to ${API?.defaults?.baseURL || ""}/settings`,
        settings
      );
  ...
  function calculateStats(data) {
    ...
    const featured = Array.isArray(data)
      ? data.filter((a) => a.featured).length
      : 0;
    const totalViews = Array.isArray(data)
      ? data.reduce((acc, a) => acc + (a.views || 0), 0)
      : 0;
  ...
  async function handleSubmit(e) {
    ...
    try {
      console.log(
        `${editingArticle ? "Updating" : "Creating"} article at ${
          API?.defaults?.baseURL || ""
        }/articles${editingArticle ? `/${editingArticle}` : ""}`
      );
      if (editingArticle) {
  ...
  async function handleDelete(id) {
    ...
    try {
      console.log(
        `Deleting article at ${API?.defaults?.baseURL || ""}/articles/${id}`
      );
  ...
  /* -----------------------
     Toggle user suspension – instant UI update
     ----------------------- */
  async function handleToggleSuspend(userId, currentSuspended) {
    ...
    try {
      ...
      alert(
        `User ${currentSuspended ? "unsuspended" : "suspended"} successfully!`
      );
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gradient-to-b from-white via-emerald-50 to-white text-gray-800 overflow-hidden">
      ...
      <main className="flex-1 md:ml-64 lg:ml-72 p-4 sm:p-6 md:p-10 max-w-7xl mx-auto overflow-y-auto">
                  ...
                  >
                    ...
                    <div className="p-3 sm:p-4 flex-1 flex flex-col">
                      ...
                      <p className="text-gray-600 text-xs sm:text-sm mt-1">
                        {a.category || "General"} •{" "}
                        {formattedDate(a.publishedAt)} • {a.views || 0} views
                      </p>
                      <p className="text-gray-600 text-xs sm:text-sm mt-1">
                      ...
                      <div className="mt-3 flex items-center justify-between">
                        <div className="flex gap-3">
                          <button
                            ...
                            className="text-emerald-603 hover:text-emerald-700 text-sm"
                          >
                            Edit
                          </button>
                          <button
        ...
        {/* Users – SUSPEND BUTTON NOW ACTIVE & STYLED */}
        {menuOpen === "users" && (
          <section className="mt-6 sm:mt-10">
            {isLoading.users && (
